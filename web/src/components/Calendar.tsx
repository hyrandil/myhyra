...
function dayColor(summary?: DailySummary) {
  if (!summary) return 'bg-slate-100 text-slate-500';
  if (summary.status === 'sick') return 'bg-rose-100 text-rose-700';
  if (summary.status === 'vacation') return 'bg-amber-100 text-amber-800';
  if (summary.status === 'open') return 'bg-rose-50 text-rose-700';
  if (summary.status === 'ok') return 'bg-emerald-50 text-emerald-700';
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-7 gap-2">
        {cells.map((cell, idx) => (
          <button
            key={idx}
            className={`rounded p-2 h-16 text-sm text-left border ${
              cell.date === selected ? 'border-sky-500 ring-1 ring-sky-300' : 'border-slate-200'
            } ${dayColor(cell.summary)}`}
          ...
          >
            <div className="flex justify-between items-start">
              ...
              {cell.summary && (
                <span className="text-xs">{cell.summary.delta >= 0 ? '+' : ''}{cell.summary.delta}m</span>
              )}
            </div>
